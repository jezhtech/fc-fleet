 ...
 */
export const generateBookingId = (date: Date | Timestamp, monthlyCount: number): string => {
  let dateObj: Date;
...
export const getNextBookingCount = async (date: Date): Promise<number> => {
  try {
    ...
    if (latestBooking.formattedId && typeof latestBooking.formattedId === 'string') {
 ...
 */
export const formatExistingBookingId = (bookingId: string, createdAt: Date | Timestamp, count: number = 1): string => {
  // If it's already in the right format, return as is
  if (typeof bookingId === 'string' && bookingId.startsWith('FC/') && bookingId.split('/').length === 4) {
    return bookingId;
  }
...
export const updateAllBookingIds = async (): Promise<number> => {
  try {
    ...
    let updateCount = 0;
    const monthCounts: Record<string, number> = {}; // To track counts by month
    ...
    querySnapshot.forEach(docSnapshot => {
      ...
      } else if (bookingData.date && typeof bookingData.date.toDate === 'function') {
