...
const SimpleZoneRenderer: React.FC<SimpleZoneRendererProps> = ({
  ...
  const renderSvgMap = () => {
    ...
    const zoneElements = availableZones.map((zone, index) => {
      ...
      const fillColor = selectedZoneId === zone.id ? '#3b82f6' : (zone.color || '#ff385c');
    ...
    return (
      <svg width={svgWidth} height={svgHeight} className="border rounded-md">
        ...
        {zoneElements}
        <text x="10" y="20" fontSize="12" fill="#666">Simple Zone Visualization (Mapbox Alternative)</text>
      </svg>
    );
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gray-50 p-4 rounded-md">
        ...
        <div className="space-y-2">
          {availableZones.length === 0 ? (
            <p className="text-gray-500">No zones available. Create a new zone to get started.</p>
          ) : (
            availableZones.map(zone => (
              <div 
                key={zone.id}
                className={`flex justify-between items-center p-3 rounded-md border ${
                  selectedZoneId === zone.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                }`}
              ...
              >
                <div>
                  ...
                  />
                  <span className="font-medium">{zone.name || 'Unnamed Zone'}</span>
                  {zone.isActive && (
      ...
      <div className="bg-yellow-50 p-4 rounded-md border border-yellow-200">
        <p className="text-sm text-yellow-800">
          <strong>Note:</strong> This is a simplified visualization due to Mapbox loading issues. 
          The actual zone coordinates will still be saved correctly even though they're displayed 
          as simple shapes here.
        </p>
      </div>
    </div>
  );
