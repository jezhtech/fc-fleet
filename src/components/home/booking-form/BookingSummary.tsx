...
const BookingSummary: React.FC<BookingSummaryProps> = ({
  ...
  const calculateEstimatedPrice = () => {
    ...
    let totalFare = basePrice + (distanceKm * perKmPrice) + (estimatedMinutes * perMinutePrice);
    ...
    <div className="space-y-6">
      <div className="rounded-md bg-green-50 p-4 border border-green-200">
        ...
        <p className="text-green-700 text-sm mt-2">
          Your booking has been initiated and is being processed. You will receive a confirmation shortly.
        </p>
      </div>
        ...
        <div className="space-y-2">
          {bookingStatuses.map((status, index) => (
            <div key={status.id} className="flex items-center">
              <div className={`h-5 w-5 rounded-full mr-3 flex items-center justify-center ${index === 0 ? 'bg-green-500' : 'bg-gray-200'}`}>
      ...
      <Card className="p-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-medium">{pickupLocation?.address || pickupLocation?.name || 'Selected Location'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Dropoff</span>
            <span className="font-medium">{dropoffLocation?.address || dropoffLocation?.name || 'Selected Location'}</span>
          </div>
          <div className="flex justify-between">
