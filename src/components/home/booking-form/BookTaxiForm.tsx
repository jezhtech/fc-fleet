...
const BookTaxiForm = () => {
  ...
  const [bookingStatus, setBookingStatus] = useState<'initiated' | 'awaiting' | 'assigned' | 'pickup' | 'dropped' | 'failed'>('initiated');
  ...
  const checkTransportTypeAvailability = async (types: TransportType[]) => {
    try {
      ...
      setAvailableTransportTypes(availableTypeIds.length > 0 ? availableTypeIds : ['economy']);
  ...
  const fetchTransportTypes = async () => {
    try {
      ...
      const fetchedTypes = types.length > 0 ? types : [
        { id: 'economy', name: 'Economy', description: 'Affordable rides for everyday use', emoji: '🚗' },
        { id: 'comfort', name: 'Comfort', description: 'More space and comfort', emoji: '🚕' },
        { id: 'suv', name: 'SUV', description: 'Spacious vehicles for groups', emoji: '🚙' },
        { id: 'premium', name: 'Premium', description: 'Luxury vehicles for special occasions', emoji: '🏎️' }
      ];
      ...
      if (fetchedVehicles.length > 0) {
        ...
        setVehicles([
          { 
            id: `${taxiTypeId}-1`, 
            taxiTypeId, 
            name: `${taxiTypeId === 'economy' ? 'Toyota Corolla' : 
                   taxiTypeId === 'comfort' ? 'Toyota Camry' : 
                   taxiTypeId === 'suv' ? 'Toyota RAV4' : 'Mercedes E-Class'}`, 
            description: 'Comfortable vehicle with AC',
            basePrice: taxiTypeId === 'economy' ? 100 : 
                      taxiTypeId === 'comfort' ? 150 : 
                      taxiTypeId === 'suv' ? 200 : 300,
            perKmPrice: taxiTypeId === 'economy' ? 10 : 
                       taxiTypeId === 'comfort' ? 15 : 
                       taxiTypeId === 'suv' ? 20 : 30,
            perMinutePrice: taxiTypeId === 'economy' ? 1 : 
                           taxiTypeId === 'comfort' ? 1.5 : 
                           taxiTypeId === 'suv' ? 2 : 3,
            capacity: taxiTypeId === 'economy' ? 4 : 
                     taxiTypeId === 'comfort' ? 4 : 
                     taxiTypeId === 'suv' ? 6 : 4,
            images: [
              taxiTypeId === 'economy' 
          ...
          { 
            id: `${taxiTypeId}-2`, 
            taxiTypeId, 
            name: `${taxiTypeId === 'economy' ? 'Honda Civic' : 
                   taxiTypeId === 'comfort' ? 'Honda Accord' : 
                   taxiTypeId === 'suv' ? 'Ford Explorer' : 'BMW 5 Series'}`, 
            description: 'Spacious with premium features',
            basePrice: taxiTypeId === 'economy' ? 120 : 
                      taxiTypeId === 'comfort' ? 170 : 
                      taxiTypeId === 'suv' ? 220 : 350,
            perKmPrice: taxiTypeId === 'economy' ? 12 : 
                       taxiTypeId === 'comfort' ? 17 : 
                       taxiTypeId === 'suv' ? 22 : 35,
            perMinutePrice: taxiTypeId === 'economy' ? 1.2 : 
                           taxiTypeId === 'comfort' ? 1.7 : 
                           taxiTypeId === 'suv' ? 2.2 : 3.5,
            capacity: taxiTypeId === 'economy' ? 4 : 
                     taxiTypeId === 'comfort' ? 5 : 
                     taxiTypeId === 'suv' ? 7 : 4,
            images: [
              taxiTypeId === 'economy' 
  ...
  const calculateEstimatedFare = () => {
    ...
    let totalFare = basePrice + (distanceKm * perKmPrice) + (timeMins * perMinutePrice);
  ...
  const saveBookingToDatabase = async (bookingData: any, paymentInfo: any, status: 'initiated' | 'awaiting' | 'failed') => {
    try {
  ...
  const handlePaymentSuccess = async (transactionId: string) => {
    ...
    if (bookingId) {
      ...
      // If saving fails, still show success but with a warning
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (step === 1) {
      ...
      const paymentDetailsObj = {
        ...
        date: pickupDate ? pickupDate.toLocaleDateString() : new Date().toLocaleDateString(),
      ...
      // Move to payment step instead of navigating away
      setStep(4);
    }
  };
        ...
        <>
          <div className="space-y-3">
            <LocationSelector 
              ...
              onChange={(value) => setBookingDetails(prev => ({ ...prev, pickup: value }))}
            ...
            <LocationSelector 
              ...
              onChange={(value) => setBookingDetails(prev => ({ ...prev, dropoff: value }))}
          ...
          <DateTimePicker 
            ...
            onTimeChange={(time) => setBookingDetails(prev => ({ ...prev, time }))}
          />
        ...
        <>
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Only transport types with available vehicles are enabled for booking.
              {availableTransportTypes.length === 0 && " No vehicles are currently available."}
            </p>
          </div>
          <TransportTypeSelector 
          ...
          <div className="flex justify-between mt-3">
            ...
            <Button 
              ...
              disabled={loading.transportTypes || loading.checkingAvailability || !selectedTaxiType || !availableTransportTypes.includes(selectedTaxiType)}
            >
              Next
            </Button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2">
            <div className="bg-white rounded-lg border p-5 mb-4">
              <h2 className="text-lg font-medium mb-4">Complete Your Payment</h2>
            ...
            <div className="bg-white rounded-lg border p-5 sticky top-5">
              ...
              <div className="space-y-3 mb-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium">{paymentDetails.vehicle?.name || "Selected Vehicle"}</span>
                </div>
                
                <div className="flex justify-between items-start">
                  <span className="text-gray-600">Pickup</span>
                  <span className="font-medium text-right max-w-[60%]">{paymentDetails.pickupLocation?.name || "Pickup Location"}</span>
                </div>
                
                <div className="flex justify-between items-start">
                  <span className="text-gray-600">Dropoff</span>
                  <span className="font-medium text-right max-w-[60%]">{paymentDetails.dropoffLocation?.name || "Dropoff Location"}</span>
                </div>
              ...
              <div className="border-t pt-3">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xl font-semibold text-fleet-red">AED {paymentDetails.amount?.toFixed(2) || "0.00"}</span>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {['initiated', 'awaiting', 'assigned', 'pickup', 'dropped'].map((status, index) => (
                <div key={status} className="flex items-center">
                  <div className={`h-5 w-5 rounded-full mr-3 flex items-center justify-center ${
                    (status === 'initiated' || (status === 'awaiting' && bookingStatus === 'awaiting')) 
                      ? 'bg-green-500' 
                      : 'bg-gray-200'
                  }`}>
                    {(status === 'initiated' || (status === 'awaiting' && bookingStatus === 'awaiting')) && (
                  ...
                  <span className={(status === 'initiated' || (status === 'awaiting' && bookingStatus === 'awaiting')) ? 'font-medium' : 'text-gray-500'}>
                    {status === 'initiated' ? 'Booking Initiated' : 
                     status === 'awaiting' ? 'Awaiting Confirmation' :
                     status === 'assigned' ? 'Driver Assigned' :
                     status === 'pickup' ? 'Pickup Done' : 'Dropped at Location'}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white rounded-lg border p-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{completedPaymentDetails.pickupLocation?.name || 'Selected Location'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Dropoff</span>
                  <span className="font-medium">{completedPaymentDetails.dropoffLocation?.name || 'Selected Location'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Date & Time</span>
                  <span className="font-medium">
                    {completedPaymentDetails.date} at {completedPaymentDetails.time}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Vehicle</span>
                  <span className="font-medium">
                    {completedPaymentDetails.vehicle?.name || 'Selected Vehicle'}
                  </span>
                </div>
                <div className="flex justify-between">
