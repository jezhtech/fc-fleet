  ...
  useEffect(() => {
    if (date) {
      ...
      if (isSameDay(date, now)) {
        ...
        const [currentTimeHours, currentTimeMinutes] = time
          .split(":")
          .map(Number);
        ...
        if (
          currentTimeHours < minHours ||
          (currentTimeHours === minHours && currentTimeMinutes < minMinutes)
        ) {
  ...
  const renderTimePickerDropdown = () => {
    ...
    return createPortal(
      <>
        ...
        >
          <div className="text-[10px] text-gray-500 font-medium mb-1.5 text-center">
            Select Time
          </div>
