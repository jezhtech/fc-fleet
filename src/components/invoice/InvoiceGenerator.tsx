...
const InvoiceGenerator: React.FC<InvoiceProps> = ({ booking, onSuccess, onError }) => {
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      ...
      const fallbackLogo = '/lovable-uploads/dbb328b3-31d8-4aa9-b1b1-3a081c9757d8.png';
        ...
        >
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 pb-4 border-b">
            <div className="flex items-center mb-4 md:mb-0">
              ...
              />
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-gray-800">First Class Fleet</h1>
                <p className="text-sm md:text-base text-gray-500">Premium Transportation Services</p>
              </div>
            </div>
            <div className="text-left md:text-right">
              <div className="inline-block border-2 border-fleet-red rounded-md px-3 py-1 mb-2">
                <h2 className="text-lg md:text-xl font-bold text-fleet-red">INVOICE</h2>
              </div>
              <p className="text-gray-700 font-semibold">{formattedBookingId}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-6">
            <div className="p-3 md:p-4 bg-gray-50 rounded-md">
              <h3 className="text-fleet-red font-semibold mb-2 border-b pb-1">Bill To:</h3>
              <p className="font-medium">{booking.customerInfo?.name || 'Customer'}</p>
              <p className="text-sm md:text-base">{booking.customerInfo?.email || 'customer@example.com'}</p>
              <p className="text-sm md:text-base">{booking.customerInfo?.phone || 'N/A'}</p>
            </div>
            <div className="p-3 md:p-4 bg-gray-50 rounded-md">
              <h3 className="text-fleet-red font-semibold mb-2 border-b pb-1">Payment Info:</h3>
              <p className="text-sm md:text-base"><span className="font-medium">Method:</span> {booking.paymentInfo?.paymentMethod || 'Online Payment'}</p>
              <p className="text-sm md:text-base"><span className="font-medium">Transaction ID:</span> {booking.paymentInfo?.transactionId || 'N/A'}</p>
              <p className="text-sm md:text-base"><span className="font-medium">Date:</span> {booking.paymentInfo?.timestamp ? 
          ...
          <div className="mb-6 p-3 md:p-4 bg-gray-50 rounded-md">
            <h3 className="text-fleet-red font-semibold mb-2 border-b pb-1">Booking Details:</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              <div>
                <p className="text-sm md:text-base"><span className="font-medium">Service Type:</span> {booking.type}</p>
                <p className="text-sm md:text-base"><span className="font-medium">Vehicle:</span> {booking.vehicle}</p>
                <p className="text-sm md:text-base"><span className="font-medium">Status:</span> {booking.status.charAt(0).toUpperCase() + booking.status.slice(1)}</p>
              </div>
              <div>
                <p className="text-sm md:text-base"><span className="font-medium">Date:</span> {format(booking.date, 'MMMM d, yyyy')}</p>
                <p className="text-sm md:text-base"><span className="font-medium">Pickup:</span> {booking.pickup}</p>
                <p className="text-sm md:text-base"><span className="font-medium">Dropoff:</span> {booking.dropoff}</p>
              </div>
              {booking.driverInfo?.name && (
                <div className="col-span-1 md:col-span-2 mt-2 pt-2 border-t">
                  <p className="text-sm md:text-base"><span className="font-medium">Driver:</span> {booking.driverInfo.name}</p>
                  {booking.driverInfo.vehicleNumber && (
                    <p className="text-sm md:text-base"><span className="font-medium">Vehicle Number:</span> {booking.driverInfo.vehicleNumber}</p>
                  )}
                </div>
              )}
            </div>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-fleet-red text-white">
                  <th className="text-left py-2 px-3 md:py-3 md:px-4 rounded-tl-md">Description</th>
                  <th className="text-right py-2 px-3 md:py-3 md:px-4 rounded-tr-md">Amount</th>
                </tr>
              </thead>
              <tbody>
                <tr className="border-b border-gray-200 bg-white">
                  <td className="py-2 px-3 md:py-3 md:px-4">{booking.type} Service - {booking.vehicle}</td>
                  <td className="text-right py-2 px-3 md:py-3 md:px-4">{booking.amount}</td>
                </tr>
              </tbody>
              <tfoot>
                <tr className="font-bold bg-gray-100">
                  <td className="py-2 px-3 md:py-3 md:px-4 rounded-bl-md">Total</td>
                  <td className="text-right py-2 px-3 md:py-3 md:px-4 rounded-br-md">{booking.amount}</td>
                </tr>
              </tfoot>
            </table>
          </div>
          ...
          {/* Terms */}
          <div className="mb-6 p-3 md:p-4 bg-gray-50 rounded-md">
            <h3 className="text-fleet-red font-semibold mb-2 border-b pb-1">Terms & Conditions:</h3>
            <ul className="text-xs md:text-sm text-gray-700 list-disc pl-5 space-y-1">
              ...
              <li>Additional charges may apply for waiting time, extra stops, or route changes.</li>
              <li>Thank you for choosing First Class Fleet for your transportation needs.</li>
            </ul>
          </div>

          {/* Footer */}
          <div className="text-center text-xs md:text-sm text-gray-500 mt-6 pt-3 border-t border-gray-300">
            <p className="font-semibold text-fleet-red">First Class Fleet LLC</p>
