  ...
  useEffect(() => {
    try {
      ...
      if (!isValidGoogleMapsApiKey(token)) {
        console.warn('Google Maps API key does not appear to be in the correct format, but will try to use it anyway');
      }
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown API key initialization error';
