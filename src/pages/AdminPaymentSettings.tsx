...
import { CreditCard, DollarSign, CheckCircle, AlertCircle, CreditCard as CCIcon, Loader2 } from 'lucide-react';
...
const AdminPaymentSettings = () => {
  ...
  useEffect(() => {
    const fetchCCavenueSettings = async () => {
      try {
        const response = await api.get('/api/payment/ccavenue/settings');
        if (response.data && response.data.success) {
          setCcavenueSettings({
            merchantId: response.data.merchantId || '',
            accessCode: response.data.accessCode || '',
            workingKey: response.data.workingKey || '',
  ...
  const handleTestConnection = async () => {
    try {
      setIsSaving(true);
      const response = await api.post('/api/payment/ccavenue/test', ccavenueSettings);
      if (response.data && response.data.success) {
        toast.success('CCAvenue connection test successful!');
      } else {
        toast.error('CCAvenue connection test failed. Please check your credentials.');
      }
    } catch (error) {
  ...
  const handleSaveIntegrationSettings = async () => {
    try {
      setIsSaving(true);
      // This will send the data to be encrypted and stored in the database
      const response = await api.post('/api/payment/ccavenue/settings', ccavenueSettings);
      if (response.data && response.data.success) {
        toast.success("CCAvenue integration settings saved successfully");
      } else {
        toast.error("Failed to save CCAvenue settings. Please try again.");
      }
    } catch (error) {
              ...
              <div className="flex items-center justify-between py-3">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Accept payments via CCAvenue payment gateway</p>
                  </div>
                </div>
                <Switch
                  checked={activePaymentMethods.ccavenue}
                  onCheckedChange={(checked) => handleTogglePaymentMethod('ccavenue', checked)}
                />
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="pt-4">
                  ...
                  <div className="flex items-center w-full max-w-xs">
                    <span className="bg-gray-100 border border-r-0 border-input px-3 py-2 rounded-l-md">{CURRENCY.symbol}</span>
                    <Input
                      type="number"
            ...
            <CardContent>
              <div className="flex items-center gap-2 border rounded-lg p-4 border-fleet-red bg-fleet-red/10">
                ...
                <div className="ml-2">
                  ...
                  <p className="text-xs text-gray-500">Accept over 200 payment methods in India</p>
                </div>
            ...
            <CardContent>
              <div className="space-y-6">
                <div className="bg-green-50 border border-green-200 rounded-md p-4 flex items-center">
                  ...
                  <p className="text-green-700 text-sm">CCAvenue is your active payment gateway.</p>
                </div>
                
                <div className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">CCAvenue Merchant ID</label>
                    <Input
                      type="text"
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-gray-500">Your credentials will be encrypted before storage</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => handleInputChange('workingKey', e.target.value)}
                    />
                    <p className="text-xs text-gray-500">This is your encryption key provided by CCAvenue</p>
                  </div>
